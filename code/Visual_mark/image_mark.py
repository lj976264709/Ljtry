...
class Image_mark:
    ...
    def mark_function_2(right_list, wrong_list, last_list, url,js_url):  # 对比结果标记
        ...
        for i in range(len(wrong_list)):
            cv2.line(images, (int(wrong_list[i][0]) - 1, int(wrong_list[i][1]) - 1),
                     (int(wrong_list[i][0]) + 1, int(wrong_list[i][1]) + 1), (255, 255, 0), 1)
            cv2.line(images, (int(wrong_list[i][0]) - 1, int(wrong_list[i][1]) + 1),
                     (int(wrong_list[i][0]) + 1, int(wrong_list[i][1]) - 1), (255, 255, 0), 1)
        for i in range(len(last_list)):
            # cv2.cv2.line(images, (int(last_list[i][0]) - 2, int(last_list[i][1])),
            #             (int(last_list[i][0]) + 2, int(last_list[i][1]) - 2), (0, 255, 0), 1)
            # cv2.cv2.line(images, (int(last_list[i][0]) - 2, int(last_list[i][1])),
            #              (int(last_list[i][0]) + 2, int(last_list[i][1]) + 2), (0, 255, 0), 1)
            # cv2.cv2.line(images, (int(last_list[i][0]) + 2, int(last_list[i][1])-2),
            #              (int(last_list[i][0]) + 2, int(last_list[i][1]) + 2), (0, 255, 0), 1)
            cv2.rectangle(images, (int(last_list[i][0]) - 1, int(last_list[i][1]-1)), (int(last_list[i][0]) +1, int(last_list[i][1])+1), (0, 255, 0), -1)
